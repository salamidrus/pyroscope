...
// TestTimeSeriesCompactAggregator_FirstSampleNotLost verifies the fix for
// https://github.com/grafana/pyroscope/issues/4937.
//
// The QueryFrontend subtracts one step from Start before setting
// InvokeRequest.StartTime, so the aggregator's startTime is (Start - step).
// The build() method must add step back before calling RangeSeries so that
// bucketing starts at the user-requested Start; otherwise samples that fall
// in the first step window land in a phantom bucket that the UI never shows.
func TestTimeSeriesCompactAggregator_FirstSampleNotLost(t *testing.T) {
	// Simulate: user requests Start=2000, End=5000, step=1s (1000ms).
	// Frontend sets InvokeRequest.StartTime = 2000 - 1000 = 1000.
	// A profile arrives at ts=1500 (inside the first visible bucket [1000,2000]).
	...
	// The first visible bucket is at timestamp 2000 (startTime + step).
	// The profile at ts=1500 must appear there, not in a phantom bucket at 1000.
	require.Len(t, series.Points, 3, "all three samples should produce visible points")
...
// TestTimeSeriesAggregator_FirstSampleNotLost is the non-compact counterpart
// of the test above, verifying the same fix for the standard aggregator path.
func TestTimeSeriesAggregator_FirstSampleNotLost(t *testing.T) {
	// Same scenario: user Start=2000, step=1s, frontend sets StartTime=1000.
	query := &queryv1.TimeSeriesQuery{
		GroupBy: []string{"service_name"},
		Step:    1.0,
	}
